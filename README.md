# 🚀 Semantic Image Retrieval Explorer

> An interactive, high-dimensional vector space exploration and nearest-neighbor visualization platform powered by **React 18**, **D3.js**, **Web Audio API**, and **t-SNE / UMAP** dimensionality reduction.
...
- **⚛️ React 18 Modular Architecture**: Pure functional component hierarchy with reactive state management and 60fps physics-eased transitions.
- **📊 1:1 Aspect Ratio Embedding Manifold**:
...
- **🌌 Upper Query Dashboard & Telemetry HUD**:
  - Live readout of active tensor dimensions (`512-DIM`), image resolution ($1024 \times 768$), FAISS query search latency ($0.82\text{ ms}$), and encoder parameters.
  - Drag-and-drop file dropzone for uploading real custom images and generating on-the-fly embeddings.
- **🔬 Non-Blocking Dimensionality Reduction**:
  - Dynamic **t-SNE** (with Perplexity slider $5 \to 50$) and **UMAP** (with Nearest Neighbors $k$ slider $5 \to 50$).
  - Decoupled projection calculations that preserve metric distance without altering fundamental FAISS nearest-neighbor rankings.
- **🔊 Web Audio API Sound Synthesizer**:
  - Futuristic, pleasant audio feedback for query selections, hover blips, and manifold updates with a toggleable `SFX: ON/OFF` switch.

---

## 🛠️ Technology Stack

- **Frontend**: React 18, JSX, Vite 5, Vanilla CSS3 (Custom Glassmorphism & Fluid Animations)
